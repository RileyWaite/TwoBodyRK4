...
orbitCount = label(pos = moon.pos + (0, 1e6, 0), text = "0", color=color.white)
...
ediffgraph = gdisplay(x = 10, y = 588, width = 950, height = 400, title = 'TBS Net Energy Spread Graph',
			     xtitle = 'Time(tick)', ytitle = 'Net Energy(J)', foreground = color.black, background = color.white)
...
with open('../SystemStates.txt', 'r') as fd:
	...
	if pastx>0 and moon.pos.x <0 or pastx<0 and moon.pos.x >0:
		orbits += .25
		orbitCount.text = str(orbits)
		
	if pasty>0 and moon.pos.y <0 or pasty<0 and moon.pos.y >0:
		orbits += .25
		orbitCount.text = str(orbits)
